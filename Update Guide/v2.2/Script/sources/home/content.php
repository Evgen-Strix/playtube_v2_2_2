    ...
    if (!empty($posts)) {
        foreach ($posts as $key => $post) {
            ...
            $html_posts .= PT_LoadPage('home/article_list', array(
                ...
                'TIME' => date($pt->config->date_style,$post->time),
...
if ($pt->config->shorts_system == 'on') {
    $videos = $db->where('is_short',1)->where('time',time() - (60 * 60),'>')->orderBy('views','DESC')->get(T_VIDEOS,5);
    if (!empty($videos) && count($videos) < 5) {
        $ids = array();
        foreach ($videos as $key => $value) {
            $ids[] = $value->id;
        }
        $new_videos = $db->where('is_short',1)->where('id',$ids,'NOT IN')->orderBy('id','DESC')->get(T_VIDEOS,5 - count($videos));
        foreach ($new_videos as $key => $value) {
            $videos[] = $value;
        }
    }
    if (empty($videos)) {
        $videos = $db->where('is_short',1)->orderBy('id','DESC')->get(T_VIDEOS,5);
    }
