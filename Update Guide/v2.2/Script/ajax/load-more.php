			...
			if (count($articles) > 0) {
			  ...
			  //       if ($pt->config->seo_link == 'on') {
			        ...
			        $posts .= PT_LoadPage('articles/list', array(
				        ...
				        'URL' => $link,
				        'TIME' => date($pt->config->date_style,$article->time),
		...
		if (!empty($videos)) {
		    ...
		    foreach ($videos as $key => $video) {
		        ...
		        if ($_POST['paid_sort'] == 'rented_videos' || $_POST['paid_sort'] == 'rented_movies') {
		            $file = 'rent';
		            $rent_video_time = date($pt->config->date_style,$video->rent_video_time + (60*60*24*2));
		            $rent_video_time_start = date($pt->config->date_style,$video->rent_video_time);
		        }
