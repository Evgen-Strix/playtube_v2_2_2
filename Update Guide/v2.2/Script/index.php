...
$forbids = ['watch', 'post', 'site-pages', 'video_text'];
if (!empty($pt->config->seo)) {
    $seo = json_decode($pt->config->seo,true);
    if (in_array($pt->page, array_keys($seo)) && !in_array($pt->page, $forbids)) {
        $pt->title       = $seo[$page]['title'];
        $pt->description = $seo[$page]['meta_description'];
        $pt->keyword     = $seo[$page]['meta_keywords'];
    }
}
