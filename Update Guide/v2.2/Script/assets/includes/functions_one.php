...
function PT_GetMedia($media = '', $is_upload = false){
    ...
    elseif (!empty($pt->config->wasabi_access_key) && $pt->config->wasabi_storage == 'on') {
        $pt->config->wasabi_site_url        = "https://s3.wasabisys.com";
        if (!empty($pt->config->wasabi_bucket_name)) {
            $pt->config->wasabi_site_url = "https://s3.wasabisys.com/".$pt->config->wasabi_bucket_name;
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
            if ($type == 1) {
                ...
                if (($pt->config->s3_upload == 'on' || $pt->config->ftp_upload == 'on' || $pt->config->spaces == 'on' || $pt->config->wasabi_storage == 'on') && !empty($last_file)) {
                ...
                if (($pt->config->s3_upload == 'on' || $pt->config->ftp_upload == 'on' || $pt->config->spaces == 'on' || $pt->config->wasabi_storage == 'on') && !empty($filename)) {
...
function PT_DeleteUser($id = 0) {
    ...
    $delete = $db->where('user_id', $id)->delete(T_ADS);
...
function PT_DeleteVideo($id = 0) {
    ...
    $s3        = (($pt->config->s3_upload == 'on' || $pt->config->ftp_upload == 'on' || $pt->config->spaces == 'on' || $pt->config->wasabi_storage == 'on' || $pt->config->amazone_s3_2 == 1)) ? true : false;
    if (strpos($get_video->thumbnail, 'upload/photos') !== false) {
...
function PT_DeleteFromToS3($filename, $config = array()) {
    ...
    if ($pt->config->s3_upload != 'on' && $pt->config->ftp_upload != 'on' && $pt->config->spaces != 'on' && $pt->config->wasabi_storage != 'on' && $pt->config->amazone_s3_2 != 1) {
        return false;
    }
    ...
    elseif ($pt->config->wasabi_storage == 'on' && !empty($pt->config->wasabi_bucket_name)) {
        require_once("assets/import/s3/vendor/autoload.php");
